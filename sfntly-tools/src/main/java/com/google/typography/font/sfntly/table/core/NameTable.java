...
public final class NameTable extends SubTableContainerTable implements Iterable<
  ...
  public static class Builder extends SubTableContainerTable.Builder<NameTable> {
    ...
    protected int subDataSizeToSerialize() {
      ...
      int size = Offset.nameRecordStart.offset + this.nameEntryMap.size()
    ...
    protected int subSerialize(WritableFontData newData) {
      ...
      // header
      newData.writeUShort(Offset.format.offset, 0);
      newData.writeUShort(Offset.count.offset, this.nameEntryMap.size());
      newData.writeUShort(Offset.stringOffset.offset, stringTableStartOffset);
      int nameRecordOffset = Offset.nameRecordStart.offset;
      int stringOffset = 0;
      for (Map.Entry<NameEntryId, NameEntryBuilder> entry : this.nameEntryMap.entrySet()) {
        // lookup table
        newData.writeUShort(nameRecordOffset + Offset.nameRecordPlatformId.offset,
            entry.getKey().getPlatformId());
        newData.writeUShort(nameRecordOffset + Offset.nameRecordEncodingId.offset,
            entry.getKey().getEncodingId());
        newData.writeUShort(nameRecordOffset + Offset.nameRecordLanguageId.offset,
            entry.getKey().getLanguageId());
        newData.writeUShort(nameRecordOffset + Offset.nameRecordNameId.offset,
            entry.getKey().getNameId());
        newData.writeUShort(nameRecordOffset + Offset.nameRecordStringLength.offset,
            entry.getValue().nameAsBytes().length);
        newData.writeUShort(
            nameRecordOffset + Offset.nameRecordStringOffset.offset, stringOffset);
        nameRecordOffset += Offset.nameRecordSize.offset;
        // string table
