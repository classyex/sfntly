...
public final class FontFactory {
  ...
  private Font[] loadCollection(InputStream is) throws IOException {
    Builder[] builders = loadCollectionForBuilding(is);
  ...
  private Builder loadSingleOTFForBuilding(InputStream is) throws IOException {
  ...
  private Builder[] loadCollectionForBuilding(InputStream is) throws IOException {
   ...
   * @throws IOException
   */
  public Builder[] loadFontsForBuilding(byte[] b) throws IOException {
    WritableFontData wfd = WritableFontData.createWritableFontData(b);
    if (isCollection(wfd)) {
      return loadCollectionForBuilding(wfd);
    }
    return new Builder[] {loadSingleOTFForBuilding(wfd, 0)};
  }
  ...
  private Font[] loadCollection(WritableFontData wfd) throws IOException {
    Builder[] builders = loadCollectionForBuilding(wfd);
    Font[] fonts = new Font[builders.length];
    for (int i = 0; i < fonts.length; i++) {
      fonts[i] = builders[i].build();
    }
  ...
  private Builder loadSingleOTFForBuilding(WritableFontData wfd, int offsetToOffsetTable)
      throws IOException {
    MessageDigest digest = null;
    if (this.fingerprintFont()) {
      // TODO(stuartg): digest of ByteArray
    }
    Builder builder = Builder.getOTFBuilder(this, wfd, offsetToOffsetTable);
    return builder;
  }

  private Builder[] loadCollectionForBuilding(WritableFontData wfd) throws IOException {
    ...
    Builder[] builders = new Builder[numFonts];
